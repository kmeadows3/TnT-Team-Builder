...
INSERT INTO skill_reference (skillset_id, name, description, phase, skill_cost) VALUES
	...
	(2, 'Field-Strip', 'When using the unjam action, model may remove two Jammed tokens instead of one.', 'Game', 0), --ID 9
	...
	(5, 'Haggler', 'Once after each game, when buying items, model may roll a D10 and reduce the cost of a single piece of equipment by the die result (min: 1). The item affected must be declared before the roll is made, but the player is not required to buy the reduced-price item. ', 'End of Game', 0), --ID 27
	(5, 'Maintainer', 'The Reliability rating of any weapon carried by the model is reduced by 1, to a minimum of 1.', 'Game', 0), --ID 28
	...
	(6, 'Bully', 'All enemies defeated by this model in close combat are knocked prone in addition to any other combat result.', 'Game', 0), --ID 33
	...
	(6, 'Strong Point', 'Model suffers no penalties to Move or Agility when wearing any type of armor.', 'Game', 0), --ID 36
	...
	(7, 'Unfazed', 'Model automatically passes all Grazed tests, but still may choose to fail as normal.', 'Game', 0), --ID 43
	...
	(8, 'Motivator', 'All friendly models within 6” of this model gain +1 to Activation tests. This does not stack with other models that have Motivator.', 'Game', 0), --ID 47
	(8, 'Tactician', 'After all models are placed at the start of the game but before initiative is determined, the player owning this model may roll 1d6 and redeploy that many members of their warband. This may only be used once per player. If two players have this ability, roll off. Winner may choose to redeploy first or second.', 'Deployment', 0), --ID 48
	...
	(9, 'Caustic Blood', 'Whenever this model is wounded or taken out-of-action, all models within 2” suffer an immediate Strength 7 hit.', 'Game', 0), --ID 53
	...
	(10, 'Flame Breath', 'Model gains a Strength 4 ranged attack that uses the Flammable rule. This ability may not be reused until the model passes a Strength test (STR/TN 10) at the beginning of its activation. This test does not cost AP.', 'Game', 0), --ID 55
	(10, 'Caustic Blood', 'Whenever this model is wounded or taken out-of-action, all models within 2” suffer an immediate Strength 7 hit.', 'Game', 0), --ID 56
	...
	(11, 'Weapon Growths', 'Model counts as being equipped with natural weapons and gains +1 to their Melee stat. However, they can only ever use one-handed weapons and equipment.', 'Game', 0), --ID 75
	...
	(12, 'Cloud Mind', '(Psychic TN 10) The psychic and all friendly models in 3” count as being in heavy cover until the beginning of the psychic’s next activation. Models already in heavy cover receive no benefit.', 'Game', 0), --ID 77
	...
	(15, 'Soft-Bellied', 'Model receives -2 to all Survival tests.', 'Game', 0), --ID 96
	...
	(15, 'Hatred', 'Model gains an enmity towards a particular person or group. When fighting against this group, gain Frenzied.', 'Game', 0), --ID 100
	...
	(15, 'Ungainly', 'This model receives a -2 penalty to all Agility tests.', 'Game', 0), --ID 107
	(16, 'Man’s Best Friend', 'All friendly K-9 and Cyber-Dog models within 6” of the Handler receive a +1 bonus to Activation Tests (MET/TN 10).', 'Game', 0), --ID 108
	...
	(16, 'Adviser', 'Any friendly models within 3” are treated as if they have the Clever skills. Multiple instances of the Clever skill may not stack.', 'Game', 0), --ID 116
...
INSERT INTO item_reference (name, cost, special_rules, rarity, is_relic, melee_defense_bonus, ranged_defense_bonus, is_shield,
	...
	('Berserker Brew', 3, '(1d6 uses per purchase) When consumed before game, unit gains +1 to Move and Melee, but becomes Frenzied. No affect on unit that already has the Frenzied special rule', 'N/A', FALSE, null, null, null, null, null, null, null, null, null, null, 'Equipment', 99), --ID 46
	...
	('Crushing Claws', 0, 'Requires Crushing Claws mutation. Always assumed to be equipped, cannot be disarmed, do not count as one-handed weapons, and do not count against carrying capacity', 'N/A', FALSE, null, null, null, null, null, 0, null, 3, null, null, 'Other', null), --ID 88
